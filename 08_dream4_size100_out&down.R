# ============================================================
# Script: 08_dream4_size100_out&down.R
#
# Description:
...
#   - CSV: DREAM4_100_out&down_results.csv (summary metrics)
#
# Dependencies:
...
for (net_id in 1:5) {
  ...
  cat("========== Running Size100 Net", net_id, "==========\n")
  ...
  wildtype <- read_tsv(file.path(data_path, paste0("insilico_size100_", net_id, "_wildtype.tsv")))
  knockouts <- read_tsv(file.path(data_path, paste0("insilico_size100_", net_id, "_knockouts.tsv")))
  knockdowns <- read_tsv(file.path(data_path, paste0("insilico_size100_", net_id, "_knockdowns.tsv")))
  combined_data <- bind_rows(wildtype, knockouts, knockdowns)
  ...
  pdf(paste0("DREAM4_Size100_Net", net_id, "_out&down_TrueGraph.pdf"), width = 7, height = 7)
  qgraph(adj_true,
         layout = "spring",
         labels = colnames(adj_true),
         title = paste0("DREAM4 Size100 Net", net_id, " True Graph(out&down)"),
         fade = FALSE, label.cex = 0.6,
         edge.width = 1,
         edge.color = "black",
         vsize = 4,
         color = "white",
  ...
  pdf(paste0("DREAM4_Size100_Net", net_id, "_out&down_pdCor.pdf"), width = 8, height = 7)
  qgraph(adj_pdcor,
         layout = "spring",
         labels = colnames(combined_data),
         title = paste0("pdCor-GGM Size100 Net", net_id, "(out&down)"),
         fade = FALSE, label.cex = 0.5,
         edge.width = TRUE,
         edge.labels = FALSE,
         vsize = 3, color = "white", border.color = "black",
  ...
  pdf(paste0("DREAM4_Size100_Net", net_id, "_out&down_Pearson.pdf"), width = 8, height = 7)
  qgraph(adj_pearson,
         layout = "spring",
         labels = colnames(combined_data),
         title = paste0("Pearson-GGM Size100 Net", net_id, "(out&down)"),
         fade = FALSE, label.cex = 0.5,
         edge.width = TRUE,
         edge.labels = FALSE,
         vsize = 3, color = "white", border.color = "black",
  ...
  pdf(paste0("DREAM4_Size100_Net", net_id, "_out&down_PR.pdf"), width = 7, height = 5)
  plot(pr_pdcor$curve[,1], pr_pdcor$curve[,2], type = "l", col = "blue", lwd = 2,
       xlab = "Recall", ylab = "Precision", main = paste0("PR Curve Size100 Net", net_id, "(out&down)"))
  lines(pr_pearson$curve[,1], pr_pearson$curve[,2], col = "red", lwd = 2)
  legend("bottomleft", legend = c("pdCor", "Pearson"), col = c("blue", "red"), lty = 1)
  dev.off()
  
  pdf(paste0("DREAM4_Size100_Net", net_id, "_out&down_ROC.pdf"), width = 7, height = 5)
  plot(roc_pdcor, col = "blue", main = paste0("ROC Curve Size100 Net", net_id, "(out&down)"))
  ...
  results_list[[net_id]] <- tibble(
    Network = paste0("Size100_Net", net_id, "(out&down)"),
...
write_csv(results_df, "DREAM4_100_out&down_results.csv")
print(results_df)
