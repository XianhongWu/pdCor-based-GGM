# ============================================================
# Script: 07_dream4_size10_out&down&mf.R
#
# Description:
...
#   - CSV: dream4_10_out&down&mf_results.csv (summary metrics)
#
# Dependencies:
...
for (net_id in 1:5) {
  ...
  cat("========== Running Net", net_id, " (wild+ko+kd+mf) ==========\n")
  ...
  wildtype <- read_tsv(file.path(data_path, paste0("insilico_size10_", net_id, "_wildtype.tsv")))
  knockouts <- read_tsv(file.path(data_path, paste0("insilico_size10_", net_id, "_knockouts.tsv")))
  knockdowns <- read_tsv(file.path(data_path, paste0("insilico_size10_", net_id, "_knockdowns.tsv")))
  multifactorial <- read_tsv(file.path(data_path, paste0("insilico_size10_", net_id, "_multifactorial.tsv")))
  ...
  pdf(paste0("DREAM4_Net", net_id, "_out&down&mf_TrueGraph.pdf"), width = 6, height = 6)
  qgraph(adj_true,
         layout = "circle",
         labels = colnames(adj_true),
         title = paste0("DREAM4 Net", net_id, " True Graph (out&down&mf)"),
  ...
  pdf(paste0("DREAM4_Net", net_id, "_out&down&mf_pdCor.pdf"), width = 7, height = 6)
  qgraph(adj_pdcor,
         layout = "circle",
         labels = colnames(combined_data),
         title = paste0("pdCor-GGM Net", net_id, " (out&down&mf)"),
  ...
  pdf(paste0("DREAM4_Net", net_id, "_out&down&mf_Pearson.pdf"), width = 7, height = 6)
  qgraph(adj_pearson,
         layout = "circle",
         labels = colnames(combined_data),
         title = paste0("Pearson-GGM Net", net_id, " (out&down&mf)"),
         fade = FALSE, label.cex = 1.2,
         edge.width = TRUE,
         edge.labels = FALSE,
         vsize = 10, color = "white", border.color = "black",
  ...
  pdf(paste0("DREAM4_Net", net_id, "_out&down&mf_PR.pdf"), width = 6.5, height = 5)
  plot(pr_pdcor$curve[,1], pr_pdcor$curve[,2], type = "l", col = "blue", lwd = 2,
       xlab = "Recall", ylab = "Precision", main = paste0("PR Curve Net", net_id, " (out&down&mf)"))
  lines(pr_pearson$curve[,1], pr_pearson$curve[,2], col = "red", lwd = 2)
  legend("bottomleft", legend = c("pdCor", "Pearson"), col = c("blue", "red"), lty = 1)
  dev.off()
  
  pdf(paste0("DREAM4_Net", net_id, "_out&down&mf_ROC.pdf"), width = 6, height = 4.5)
  plot(roc_pdcor, col = "blue", main = paste0("ROC Curve Net", net_id, " (out&down&mf)"))
  ...
  results_list[[net_id]] <- tibble(
    Network = paste0("Net", net_id, "_out&down&mf"),
...
write_csv(results_df, "dream4_10_out&down&mf_results.csv")
print(results_df)
