# ============================================================
# Script: 06_dream4_size10_out&down.R
#
# Description:
#   DREAM4 Size-10 benchmark (5 networks). For each network, this
#   script loads wild-type, knockouts, and knockdowns, constructs:
...
#
# Inputs:
#   - DREAM4 training data folders for Size-10 networks (1..5)
#   - DREAM4 gold standard TSV files (size10_*_goldstandard.tsv)
#   * Paths are set below; adjust to your local machine.
#
# Outputs (per network):
#   - PDFs: True graph, pdCor graph, Pearson graph, PR curve, ROC curve
#   - CSV: dream4_10_out&down_results.csv (summary metrics)
#
# Dependencies:
...
# ========= DREAM4 Size10 batch loop ==========
results_list <- list()

for (net_id in 1:5) {
  cat("========== Running Net", net_id, "==========\n")

  # ---- 1) Paths ----
  data_path <- paste0("/Users/wuxianhong/Desktop/Thesis/Data-Dream4/Size 10/DREAM4 training data/insilico_size10_", net_id)
  gold_file <- paste0("/Users/wuxianhong/Desktop/Thesis/Data-Dream4/Size 10/DREAM4 gold standards/insilico_size10_", net_id, "_goldstandard.tsv")

  # ---- 2) Read & combine datasets ----
  wildtype   <- read_tsv(file.path(data_path, paste0("insilico_size10_", net_id, "_wildtype.tsv")),   col_names = TRUE)
  knockouts  <- read_tsv(file.path(data_path, paste0("insilico_size10_", net_id, "_knockouts.tsv")),  col_names = TRUE)
  knockdowns <- read_tsv(file.path(data_path, paste0("insilico_size10_", net_id, "_knockdowns.tsv")), col_names = TRUE)
  combined_data <- rbind(wildtype, knockouts, knockdowns)
  ...
  # Align node order with data columns
  adj_true <- adj_true[colnames(combined_data), colnames(combined_data)]
  cat("Gold standard edges: ", sum(adj_true) / 2, "\n")

  pdf(paste0("DREAM4_Net", net_id, "_out&down_TrueGraph.pdf"), width = 6, height = 6)
  qgraph(adj_true,
         layout = "circle",
         labels = colnames(adj_true),
         title = paste0("DREAM4 Net", net_id, " True Graph (out&down)"),
         fade = FALSE, label.cex = 1.4,
         edge.width = 3,
         edge.color = "black",
         vsize = 10,
         color = "white",
  ...
  pdf(paste0("DREAM4_Net", net_id, "_out&down_pdCor.pdf"), width = 7, height = 6)
  qgraph(adj_pdcor,
         layout = "circle",
         labels = colnames(combined_data),
         title = paste0("pdCor-GGM Net", net_id, " (out&down)"),
         fade = FALSE, label.cex = 1.2,
         edge.width = TRUE,
         edge.labels = FALSE,
         vsize = 10, color = "white", border.color = "black",
         label.color = "black", directed = FALSE)
  dev.off()
  ...
  pdf(paste0("DREAM4_Net", net_id, "_out&down_Pearson.pdf"), width = 7, height = 6)
  qgraph(adj_pearson,
         layout = "circle",
         labels = colnames(combined_data),
         title = paste0("Pearson-GGM Net", net_id, " (out&down)"),
         fade = FALSE, label.cex = 1.2,
         edge.width = TRUE,
         edge.labels = FALSE,
         vsize = 10, color = "white", border.color = "black",
  ...
  pr_pdcor    <- pr.curve(scores.class0 = y_pdcor[y_true == 1],
                          scores.class1 = y_pdcor[y_true == 0],
                          curve = TRUE)
  pr_pearson  <- pr.curve(scores.class0 = y_pearson[y_true == 1],
                          scores.class1 = y_pearson[y_true == 0],
                          curve = TRUE)

  pdf(paste0("DREAM4_Net", net_id, "_out&down_PR.pdf"), width = 6.5, height = 5)
  plot(pr_pdcor$curve[,1], pr_pdcor$curve[,2], type = "l", col = "blue", lwd = 2,
       xlab = "Recall", ylab = "Precision", main = paste0("PR Curve Net", net_id, " (out&down)"))
  lines(pr_pearson$curve[,1], pr_pearson$curve[,2], col = "red", lwd = 2)
  legend("bottomleft", legend = c("pdCor", "Pearson"), col = c("blue", "red"), lty = 1)
  dev.off()

  pdf(paste0("DREAM4_Net", net_id, "_out&down_ROC.pdf"), width = 6, height = 4.5)
  plot(roc_pdcor, col = "blue", main = paste0("ROC Curve Net", net_id, " (out&down)"))
  ...
  results_list[[net_id]] <- tibble(
    Network = paste0("Net", net_id, "_out&down"),
...
write_csv(results_df, "dream4_10_out&down_results.csv")
print(results_df)
